...
| 5.4 | "OTP stride-4 chunks reconstruct a byte-addressable register space; mismatches vs register dump = probe low-drive failure." | PROVISIONAL-ONLY: OTP chunks carry the same prefix-byte framing; byte-offset reconstruction is invalid. The physical-layer advice stands for genuine 0xFF tails, but the DID/VID "mismatch" between datasets was apples-to-oranges. Cross-check kept as a consistency tripwire, wording fixed in v0.9.2+ (TODO: reword the validator message). |
